...
export function resolveReportInstantRange(params: DateRangeParams, timezone: string): { start: Date; end: Date; startLabel: string; endLabel: string } {
  ...
  const defaultEnd = { y: now.getUTCFullYear(), m: now.getUTCMonth(), d: now.getUTCDate() }
  ...
  const startLocalMidnight = new Date(Date.UTC(startParts.y, startParts.m, startParts.d))
  // end is inclusive of the whole day, so the instant boundary is the *next* day's midnight
  const endLocalMidnightNextDay = new Date(Date.UTC(endParts.y, endParts.m, endParts.d + 1))
