"use client"
// The clinic admin's copy of app/console/clinics/[id]/branches/new/new-branch-form.tsx.
// Not shared because a server component cannot pass a function to a client
// component, so the form must import its own server action — and this one
// takes no clinic id (the action derives it from the session; see ../../actions.ts).
...
import { createOwnClinicBranchAction } from "../../actions"
...
export function NewBranchForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setPending(true)
    setError(null)
    const res = await createOwnClinicBranchAction({ ...form, operatingHours: toOperatingHours(hours) })
    setPending(false)
    if (!res.ok) {
      setError(res.error)
      return
        ...
        <CardContent className="py-6">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <Button type="button" asChild>
              <Link href="/console/clinic">Back to branches</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
